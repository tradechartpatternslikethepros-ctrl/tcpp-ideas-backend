 ...
 * + multi-user auth (JWT) + price/status engine (EL/SL/TP)
 * + email triggers + SSE live feed
 */
...
/* IMPORTANT:
   - API_TOKEN   = legacy single static token (admin / fallback)
   - JWT_SECRET  = per-user token signer (what Wix issues)
*/
...
/* Optional webhook fallback (Wix function / Zapier / etc) */
...
/*
   ...
   1. Per-user JWT (preferred)
      - Bearer <token> where token was minted from Wix using JWT_SECRET
      - Decoded form: { sub, email, name, role }

   2. Legacy static API_TOKEN (fallback/admin)
      - Bearer <API_TOKEN>

   We attach req.user = { id, email, name, role }
*/
...
function buildUserFromReq(req){
  const tok = readBearer(req) || readQueryToken(req);
  if (!tok) return null;
  const uJWT = decodeJWT(tok);
  if (uJWT) return uJWT;
  const uLEG = decodeLegacy(tok);
  if (uLEG) return uLEG;
  return null;
}
function requireAuth(req,res,next){
  const u = buildUserFromReq(req);
  if (!u) return err(res, 401, 'Unauthorized');
  req.user = u;
  next();
}
function sseAuthOK(req){
  // allow open SSE only if:
  // - no auth configured at all, OR
  // - provided token is valid JWT or legacy token
  if (!JWT_SECRET && !API_TOKEN) return true;
  const u = buildUserFromReq(req);
  return !!u;
}
...
function sseTo(res, event, data){
  res.write(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`);
}
function sseSend(event, data){
  const line = `event: ${event}\ndata: ${JSON.stringify(data)}\n\n`;
  for (const c of clients) {
    try { c.res.write(line); } catch(_e){}
  }
}
...
function evalStatus(idea){
  ...
  if (dir === 'long') {
    if (Number.isFinite(sl) && p <= sl) {
      m.statusLight='red';   m.statusNote='Stop hit'; m.hitStop = true;
    } else if (Number.isFinite(firstTarget) && p >= firstTarget) {
      m.statusLight='blue';  m.statusNote='Target reached';
      if (m.hitTargetIndex == null) m.hitTargetIndex = 0;
    } else if (p >= el) {
      m.statusLight='green'; m.statusNote='Above EL';
    } else {
      m.statusLight='orange';m.statusNote='Below EL';
    }
  } else if (dir === 'short') {
    if (Number.isFinite(sl) && p >= sl) {
      m.statusLight='red';   m.statusNote='Stop hit'; m.hitStop = true;
      ...
      m.statusLight='green'; m.statusNote='Below EL';
    } else {
      m.statusLight='orange';m.statusNote='Above EL';
    }
  } else {
...
function mapSymbol(raw){
  ...
  // Binance spot: assume USDT pair
  if (ex === 'BINANCE') {
    return { kind:'binance', b: code.replace('USD','USDT') };
  }

  // FX pair like EURUSD => Yahoo "EURUSD=X"
  if (/^[A-Z]{6}$/.test(code)) {
    return { kind:'yahoo', y:`${code}=X` };
  }
...
async function fetchPrice(symbol){
  ...
  if (c && (Date.now()-c.at) < PRICE_STALE_SEC*1000) {
    return c.price;
  }
  ...
  if (m.kind === 'binance') {
    try {
      const j = await httpJSON(
        `https://api.binance.com/api/v3/ticker/price?symbol=${encodeURIComponent(m.b)}`
      );
  ...
  if (!Number.isFinite(price)) {
    try {
      const y = await httpJSON(
        `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(m.y)}`
      );
...
function recipientsFor(kind, item){
  // force override for debugging/broadcasts
  const forced = splitEmails(EMAIL_FORCE_ALL_TO);
  if (forced.length) {
    return { to: forced[0], bcc: forced.slice(1) };
  }

  const author = (item?.authorEmail||'').toLowerCase();
  const to  = EMAIL_RX.test(author)
    ? author
    : (EMAIL_BCC_ADMIN[0] || undefined);
...
function bullets(s){
  ...
  return `<ul style="margin:8px 0 0 20px;padding:0">${parts.map(li=>(
    `<li style="margin:6px 0;line-height:1.45">${esc(li)}</li>`
  )).join('')}</ul>`;
}

function emailShell({ title, symbol, levelsHTML, planHTML, imgUrl, ctaHref, badgeText }){
  const logo = absUrl(LOGO_URL);
  const isDark = EMAIL_THEME !== 'light';
  ...
  <tr><td align="center">
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:680px;background:${cardBg};border:1px solid ${border};border-radius:18px;overflow:hidden">
      ...
      <tr><td style="padding:8px 18px 0">
        <img src="${esc(imgUrl)}" width="640" style="display:block;width:100%;max-width:640px;border:1px solid ${border};border-radius:14px" alt="">
      </td></tr>` : ''}
      ...
      <tr><td style="padding:16px 18px;color:${text};font-family:Inter,Segoe UI,Roboto,Arial">
        ...
        ${symbol ? `<div style="margin:2px 0 10px 0">
          <span style="display:inline-block;background:#0fd5ff12;color:#0fd5ff;border:1px solid #0fd5ff38;padding:6px 10px;border-radius:999px;font-weight:800;font-size:12px;letter-spacing:.2px">${esc(symbol)}</span>
        </div>`:''}
...
function levelsHTMLFromIdea(it){
  ...
  if (Array.isArray(m.targets) && m.targets.length) {
    parts.push(`TPs: ${m.targets.join(', ')}`);
  }
  return bullets(parts.join('; '));
}
function planHTMLFromIdea(it){
  const s = String(it.take||'').trim();
  return s
    ? `<p style="margin:0;line-height:1.55">${esc(s)}</p>`
    : '';
}
...
async function httpFallbackSend(payload){
  if (!EMAIL_HTTP_FALLBACK_URL) {
    throw new Error('HTTP fallback not configured');
  }
  return new Promise((resolve,reject)=>{
    const data = JSON.stringify(payload);
    const req = (EMAIL_HTTP_FALLBACK_URL.startsWith('https:')?https:http).request(
      EMAIL_HTTP_FALLBACK_URL,
      {
        method:'POST',
        headers:{
...
async function sendEmailSmart(kind, item){
  ...
  const title = (kind==='post')
    ...
    : (kind==='stop'
        ? `STOP hit — ${item.symbol}`
        : `TARGET hit — ${item.symbol}`);

  const html = emailShell({
    title,
    symbol: item.symbol || '',
    levelsHTML: levelsHTMLFromIdea(item),
    planHTML:   planHTMLFromIdea(item),
    imgUrl:     firstImageUrl(item),
    ctaHref:    `${SITE_URL}/trading-dashboard?idea=${encodeURIComponent(item.id)}`,
    badgeText:  (kind==='post' ? 'New Post' : (kind==='stop' ? 'STOP' : 'TARGET'))
  });

  try {
    return await smtpSend({ subject:title, html, to, bcc });
  } catch(e) {
    if (EMAIL_HTTP_FALLBACK_URL) {
      try {
        return await httpFallbackSend({ kind, item });
      } catch(_e2){}
    }
    throw e;
  }
...
/* Trigger emails when status flips to hitStop / hitTargetIndex */
async function maybeTriggerEmails(prev, curr){
  ...
  if (cm.hitStop && !pm.hitStop && !cm.notified?.stop) {
    ...
    try { await sendEmailSmart('stop', curr); } catch(_e){}
  ...
  if (currIdx >=0 && currIdx > prevIdx) {
    ...
    if (!cm.notified.targets[currIdx]) {
      try { await sendEmailSmart('target', curr); } catch(_e){}
...
// public serving of uploaded images
app.use(
  '/uploads',
  express.static(UPLOAD_DIR, {
    ...
    maxAge:'30d',
    setHeaders:res=>{
      // allow Wix frontends / dashboards to embed
...
app.post('/ideas', requireAuth, async (req,res)=>{
  ...
  // attach author from req.user so nobody can spoof
  const body = { ...req.body };
  body.authorId    = req.user.id    || body.authorId;
  body.authorName  = req.user.name  || body.authorName;
  body.authorEmail = req.user.email || body.authorEmail;

  const it = normalizeIdea(body);

  if (!it.imageUrl && it.media?.[0]?.url) {
...
// update idea (auth required — owner or admin)
app.patch('/ideas/:id', requireAuth, async (req,res)=>{
  ...
  if (!isAdmin(req) && it.authorId !== req.user.id) {
  ...
  // fire STOP/TP emails if state changed
  await maybeTriggerEmails(prev, it).catch(()=>{});

  ok(res, { item: pub });
});

// delete idea (auth required — owner or admin)
...
/* ------------------------ LIKES ----------------------- */
/*
   Likes are now tied to req.user, so you can't spoof
...
async function likeHandler(req,res){
  ...
  const userKey = req.user.id || req.user.email || 'anon';
  const displayName = req.user.name || 'Member';

  it.likes ||= { count:0, by:{} };
  it.likes.by ||= {};

  const already = !!it.likes.by[userKey];

  if (action === 'like' && !already) {
    it.likes.by[userKey] = { at: nowISO(), name: displayName };
  } else if (action === 'unlike' && already) {
    delete it.likes.by[userKey];
  }
...
async function commentEdit(req,res){
  ...
  if (!isAdmin(req) && c.authorId !== req.user.id) {
    return err(res, 403, 'Forbidden');
  }
...
async function commentDelete(req,res){
  ...
  const list = it.comments?.items || [];
  const idx = list.findIndex(x=> String(x.id)===String(req.params.cid));
...
app.post('/email/post', requireAuth, async (req,res)=>{
  try {
    ...
    // backfill author from req.user so emails are attributable
    if (!item.authorEmail && req.user.email) item.authorEmail = req.user.email;
    if (!item.authorName  && req.user.name)  item.authorName  = req.user.name;

    await sendEmailSmart('post', item);
    ok(res,{ok:true});
  } catch(e){
    err(res, 500, String(e.message||e));
  }
...
app.post(['/subscribe','/api/subscribe','/email/subscribe'], requireAuth, async (req,res)=>{
  try{
    const name  = (req.body?.name  || req.user.name  || 'Member').trim();
    const email = (req.body?.email || req.user.email || '').trim().toLowerCase();

    if (!EMAIL_RX.test(email)) return err(res, 400, 'Invalid email');

    const subs = await loadSubs();
    if (!subs.subs.find(s=> s.email===email)) {
      subs.subs.push({ email, name, at: nowISO() });
    }
    await saveSubs(subs);
...
/* ------------------------ UPLOAD ---------------------- */
app.post('/upload', requireAuth, (req,res,_next)=>{
  upload.single('file')(req,res, async (e)=>{
    ...
    try{
      ...
      err(res, 500, String(ex.message||ex));
    }
  });
});
...
async function maybeAutoSetStatus(it){
  ...
  if (m.hitStop && it.status!=='lost') {
    it.status='lost'; changed = true;
  } else if (
    Number.isFinite(m.hitTargetIndex)
    && m.hitTargetIndex>=0
    && it.status!=='won'
  ) {
    it.status='won'; changed = true;
  } else if (m.last!=null && m.entry!=null && it.status!=='live') {
    it.status='live'; changed = true;
  }
...
async function pollOnce(){
  ...
  try{
    ...
    for (const it of db.ideas) {
      ...
      const watch = (
        it.metrics.entry != null ||
        it.metrics.stop  != null ||
        ((it.metrics.targets||[]).length>0)
      );
      if (!watch) continue;
...
(async function start(){
  ...
  app.listen(PORT, ()=>{
    console.log(
      `[ideas-backend] listening on :${PORT} env=${NODE_ENV} data=${DATA_FILE}`
    );
  });
})();
